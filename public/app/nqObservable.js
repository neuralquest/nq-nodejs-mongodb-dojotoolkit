...
var Observable = function(/*Store*/ store){
	...
	var newResults = originalQuery.apply(store, [query]);
	when(newResults, function(newResultsArray){
		var arraysDiffer = resultsArray.length != newResultsArray.length;
		if(!arraysDiffer){
			for(i = 0, l = resultsArray.length; i < l; i++){
				var oldArrayObj = resultsArray[i];
				var newArrayObj = newResultsArray[i];
				for(var key in oldArrayObj){
					if(oldArrayObj[key] != newArrayObj[key]){
						arraysDiffer = true;
						break;
					}
				}
				if(arraysDiffer) break;
			}
		}
		if(arraysDiffer){
			//console.log('New UPDATER', query, resultsArray, newResultsArray);
			resultsArray = newResultsArray;
			var copyListeners = listeners.slice();
			for(i = 0;listener = copyListeners[i]; i++){
				listener(changed, -2, -1);
			}
