...
//			fx.fadeOut({node: 'loadingOverlay',	onEnd: function(node){domStyle.set(node, 'display', 'none');}}).play();
	...
	function interpritHash(hash, lvl){
		...
		return when(_nqDataStore.get(PAGE_MODEL_VIEWS_ID+'/'+state.viewId), function(view){
			var viewPanePaneCreated = null;
			if(view[ACCORTAB_ATTR_ID]==ACCORDION_ID) viewPanePaneCreated = createAccordionInBorderContainer(parentViewPane, view, state.tabId, level);
			else viewPanePaneCreated = createTabs(parentViewPane, view, state.tabId, level);
			return when((viewPanePaneCreated), function(selectedTab){//returns the selected tab!
				parentViewPane.resize();//this is a must
				when(createWidgetProvideData(selectedTab, level), function(widgets){
					return when(_nqDataStore.getChildren(selectedTab, [WIDGETS_VIEW_ID]), function(widgets){
						for(var i=0;i<widgets.length;i++){
							var widgetObj = widgets[i];
							when(createWidgetProvideData(widgetObj, level), function(widget){
				...
				//we do not have to wait for the widgets to be completed.
				//we can continue with a recurssive call to interpritHash
	...
	//////////////////////////////////////////////////////////////////////////////
	function createAccordionInBorderContainer(parentContentPane, view, selectedTabId, level){
		var ACCORTAB_VIEW_ID = 1802;
		return when(_nqDataStore.getChildren(view, [ACCORTAB_VIEW_ID]), function(tabs){
			var selectedTab = tabs[0];
			// if the border container already exists we can simply return the tabs
			if(registry.byId('borderContainer'+view.id.split('/')[1])) {
				for(var i=0;i<tabs.length;i++){
					var tt = tabs[i];
					var ttId = tt.id.split('/')[1];
					if(ttId == selectedTabId) selectedTab=tt; 
				}
				return selectedTab;
			}
			...
			var borderContainer = new BorderContainer( {
				'id' : 'borderContainer'+view.id.split('/')[1],
				'region' : 'center',
				'design' : design,
				'persist' : true,
				//'class': 'noOverFlow'
			...
			//createviewPanePane(leftPane, 0, view.id);
			//createviewPanePane(centerPane, level+1);
		
			var accordianContainer;
			if(tabs.length==1){// this is really only to have palce to store viewPane+view.id. Is there a better way?
				accordianContainer = new ContentPane( {
					'id' : 'viewPane'+view.id.split('/')[1],
					'region' : 'center',
//					'style' : {width: '100%',height: '100%',overflow: 'hidden',padding: '0px', margin: '0px'}
				});
			}
...
//					'style' : {width: '100%',height: '100%',overflow: 'hidden',padding: '0px', margin: '0px'}
			...
			for(var i=0;i<tabs.length;i++){
				...
				if(tabId==selectedTabId) selectedTab = tab;//use this one for the return value.
...
//					'viewPane': view.id,
				...
				accordianContainer.watch("selectedChildWidget", function(name, oval, nval){
				    ...
				    setHashTabId(level, tabId); // this will trigger createWidgetProvideData
				});
			}
			borderContainer.startup();
console.log('accordion container created', level);		
			return selectedTab;
		}, errorDialog);
	}
	//////////////////////////////////////////////////////////////////////////////
	// Add a tab container
	//////////////////////////////////////////////////////////////////////////////
	function createTabs(patentPane, view, selectedTabId, level){
		var ACCORTAB_VIEW_ID = 1802;
		return when(_nqDataStore.getChildren(view, [ACCORTAB_VIEW_ID]), function(tabs){
			var selectedTab = tabs[0];//use this one for the return value.
			// if the ContentPane already exists we can simply return the tabs
			if(registry.byId('viewPane'+view.id.split('/')[1])) {
				for(var i=0;i<tabs.length;i++){
					var tt = tabs[i];
					var ttId = tt.id.split('/')[1];
					if(ttId == selectedTabId) selectedTab=tt; 
...
console.log('tab container created', level);		
	...
	//////////////////////////////////////////////////////////////////////////////
	function createWidgetProvideData(widgetObj, level){
...
console.log('using tab node', level, tabNode);		

		// if the widget already exists we can simply return widgets
		var widget = registry.byId('widget'+widgetObj.id);
...
/*	
	function XinterpritHash(){
		console.log('hash: ', hash());
		var hashArr = hash().split('.');
		var levels = Math.ceil(hashArr.length/3);
		var promisses = [true];
		for(var level = 0; level<levels; level++){
			when(promisses[level], function(results){
				promisses[level+1] = buildLevelPageParts(level);
			}, errorDialog);		
		}
return;
		for(var level = 0; level<levels; level++){
			when(buildLevelPageParts(level), function(results){
				return results;
			}, function(err){console.dir(err); new dijit.Dialog({title: "Interprit Hash Error", extractContent: true, content: err.message}).show();});
		}
		when(buildLevelPageParts(0), function(widgets){
			return when(buildLevelPageParts(1), function(widgets){
				return when(buildLevelPageParts(2), function(widgets){
					return true;
				}, function(err){console.dir(err); new dijit.Dialog({title: "Interprit Hash Error", extractContent: true, content: err.message}).show();});
			}, function(err){console.dir(err); new dijit.Dialog({title: "Interprit Hash Error", extractContent: true, content: err.message}).show();});
		}, function(err){console.dir(err); new dijit.Dialog({title: "Interprit Hash  Error", extractContent: true, content: err.message}).show();});
 ...
 //////////////////////////////////////////////////////////////////////////////
	...
	function interpritHash(){
		XinterpritHash();return;
		var hashArr = hash().split('.');
		var levels = Math.ceil(hashArr.length/3);
		for(var level = 0; level<levels; level++){
			var state = getState(level);
			if(state.viewId && !registry.byId('viewPane'+state.viewId)){
				var viewDef = _nqSchemaStore.get(state.viewId);
				var parentContentPane = findParentContainer(viewDef);
				if(viewDef.containerType == 'Accordion') createAccordionInBorderContainer(parentContentPane, state.viewId, state.tabId, level);
	...
	//////////////////////////////////////////////////////////////////////////////
	function createAccordionInBorderContainer(parentContentPane, parentTabOrViewId, selectedTabId, level){
		...
		borderContainer.addChild(centerPane);
		parentContentPane.containerNode.appendChild(borderContainer.domNode); //appendChild works better than attaching through create
		//createviewPanePane(leftPane, 0, parentTabOrViewId);
		//createviewPanePane(centerPane, level+1);
		
		var tabsArr = _nqSchemaMemoryStore.query({parentViewId: parentTabOrViewId, entity: 'tab'});//get the tabs		 
		var accordianContainer;
		if(tabsArr.length==1){// this is really only to have palce to store viewPane+parentTabOrViewId. Is there a better way?
			accordianContainer = new ContentPane( {
				'id' : 'viewPane'+parentTabOrViewId,
				'region' : 'center',
				'persist' : true
//				'style' : {width: '100%',height: '100%',overflow: 'hidden',padding: '0px', margin: '0px'}
			});
